...
def get_episodes(base_url, season, series_name, logo, m3u8_content, result):
    ...
    for episode in range(1, 101):
        ...
        if code_match:
            ...
            extinf_title = f"{series_name} S{season}E{episode}"
            m3u8_line = (
                f'#EXTINF:-1 tvg-id="vod.tr" tvg-name="{extinf_title}" '
                f'tvg-logo="{logo}" group-title="DMAX BELGESELLER",{extinf_title}\n'
                f'{m3u8}\n'
            )
            ...
            print(f"[{extinf_title}] {m3u8}", flush=True)
            episode_count += 1
